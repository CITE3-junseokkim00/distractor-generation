...
def clean_text(text):
    ...
    if lcode in ['ko']: # korean
        text = regex.sub(u"[^ \r\n\p{Hangul}.?!0-9]", " ", text) # Replace unacceptable characters with a space.
...
def word_segment(sent):
    ...
    global lcode
    if lcode in ['ko']:
        words = [word for word, _ in okt.pos(sent)]
    elif lcode in ['ja']:
...
def build_corpus():
    ...
    with codecs.open("data/{}withNum.txt".format(lcode), 'w', 'utf-8') as fout:
        i = 1
        j = 1
